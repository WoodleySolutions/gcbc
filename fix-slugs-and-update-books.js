...
async function fixAndUpdateBooks() {
  console.log('Fixing slugs and updating 2026 books with themes and descriptions...\n');

  // Get all 2026 books
  const books = await client.fetch('*[_type == "monthlyBook" && year == 2026]');

  for (const book of books) {
    const updateData = bookData[book.title];
    ...
    try {
      ...
      console.log(`   Theme: ${updateData.theme}`);
      console.log('');
    } catch (error) {
  ...
  console.log('✅ All 2026 books have been updated!');
}

fixAndUpdateBooks().catch(console.error);
